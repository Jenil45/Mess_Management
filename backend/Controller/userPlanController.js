...
export const getConsent = asyncHandler(async (req , res) => {
    ...
    //         {
    //             $unwind : {path:'$isavailable'}
    //         },
    //                     // {
    //         //     $group : {
    //         //         _id: "$_id",
    //         //         consent : { $push : '$isavailable.'}
    //         //     }
    //         // },
    //     ]
    //     )

    const getConsent = await UserPlan.findOne({userId , planId , "isavailable.date":date} , {
        _id: 0, isavailable: {$elemMatch: {date: date}}
        })
...
export const getUserTodayPlan = asyncHandler(async (req , res) => {
    ...
    "end_date":{$gte:today_date} , "isavailable.date":today_date},{
        _id:0,
        userId:1,planId:1,fees:1,fee_status:1,isavailable:{$elemMatch:{date:today_date}}
    })
    // console.log(user);
    if (!user) {
        return res.status(400).json({ message: 'No users found' })
